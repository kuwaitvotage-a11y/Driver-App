class XenditModel {
  ...
  XenditModel.fromJson(Map<String, dynamic> json) {
    ...
    // Safely parse all list fields - handle String, List, or null
    var banksRaw = json['available_banks'];
    if (banksRaw != null && banksRaw is List && banksRaw.isNotEmpty) {
      availableBanks = <AvailableBanks>[];
      for (var v in banksRaw) {
        availableBanks!.add(AvailableBanks.fromJson(v));
      }
    }
    var retailRaw = json['available_retail_outlets'];
    if (retailRaw != null && retailRaw is List && retailRaw.isNotEmpty) {
      availableRetailOutlets = <AvailableRetailOutlets>[];
      for (var v in retailRaw) {
        availableRetailOutlets!.add(AvailableRetailOutlets.fromJson(v));
      }
    }
    var ewalletsRaw = json['available_ewallets'];
    if (ewalletsRaw != null && ewalletsRaw is List && ewalletsRaw.isNotEmpty) {
      availableEwallets = <AvailableEwallets>[];
      for (var v in ewalletsRaw) {
        availableEwallets!.add(AvailableEwallets.fromJson(v));
      }
    }
    var qrRaw = json['available_qr_codes'];
    if (qrRaw != null && qrRaw is List && qrRaw.isNotEmpty) {
      availableQrCodes = <AvailableQrCodes>[];
      for (var v in qrRaw) {
        availableQrCodes!.add(AvailableQrCodes.fromJson(v));
      }
    }
    var debitsRaw = json['available_direct_debits'];
    if (debitsRaw != null && debitsRaw is List && debitsRaw.isNotEmpty) {
      availableDirectDebits = <AvailableDirectDebits>[];
      for (var v in debitsRaw) {
        availableDirectDebits!.add(AvailableDirectDebits.fromJson(v));
      }
    }
    var paylatersRaw = json['available_paylaters'];
    if (paylatersRaw != null &&
        paylatersRaw is List &&
        paylatersRaw.isNotEmpty) {
      availablePaylaters = <AvailablePaylaters>[];
      for (var v in paylatersRaw) {
  ...
  Map<String, dynamic> toJson() {
    ...
    if (availableQrCodes != null) {
      data['available_qr_codes'] =
          availableQrCodes!.map((v) => v.toJson()).toList();
    }
    if (availableDirectDebits != null) {
      data['available_direct_debits'] =
          availableDirectDebits!.map((v) => v.toJson()).toList();
    }
    if (availablePaylaters != null) {
      data['available_paylaters'] =
          availablePaylaters!.map((v) => v.toJson()).toList();
    }
