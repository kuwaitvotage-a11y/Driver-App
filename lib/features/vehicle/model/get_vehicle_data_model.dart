class GetVehicleDataModel {
  ...
  GetVehicleDataModel.fromJson(Map<String, dynamic> json) {
    ...
    vehicleData =
        json['data'] != null ? VehicleData.fromJson(json['data']) : null;
  }
...
class VehicleData {
  ...
  VehicleData.fromJson(Map<String, dynamic> json) {
    ...
    minimumDeliveryChargesWithin =
        json['minimum_delivery_charges_within_km'].toString();
    // Handle zone_id which can be a List, String (comma-separated), or null
    var zoneIdRaw = json['zone_id'];
    if (zoneIdRaw == null || zoneIdRaw == '' || zoneIdRaw == '[]') {
      zone_id = [];
    } else if (zoneIdRaw is List) {
      zone_id = zoneIdRaw;
    } else if (zoneIdRaw is String) {
      // Parse comma-separated string like "9,10" into list
      zone_id = zoneIdRaw.split(',').where((e) => e.trim().isNotEmpty).toList();
    } else {
      zone_id = [];
    }
  }
